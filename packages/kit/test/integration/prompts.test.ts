...
describe('Kit Integration Tests', () => {
  ...
  describe('function signatures', () => {
    ...
    it('number should accept string message', async () => {
      ...
      // Since number prompt starts with "0" by default, just submit it
      await new Promise(resolve => setTimeout(resolve, 10));
      mockStreams.sendKey({ name: 'return' });
    ...
    it('number should accept options object', async () => {
      ...
      const result = await promptPromise;
      expect(typeof result).toBe('number');
      expect(result).toBe(25); // Default value
    });
  });
  ...
  describe('type inference', () => {
    it('should infer string type for text prompt', async () => {
      ...
      // TypeScript should infer Promise<string>
      const typeCheck: Promise<string> = promptPromise;
    ...
    it('should infer boolean type for confirm prompt', async () => {
      const promptPromise = confirm('Continue?');
      // TypeScript should infer Promise<boolean>
      const typeCheck: Promise<boolean> = promptPromise;
      expect(typeCheck).toBeDefined();
    ...
    it('should infer number type for number prompt', async () => {
      ...
      // TypeScript should infer Promise<number>
      const typeCheck: Promise<number> = promptPromise;
      expect(typeCheck).toBeDefined();
    ...
    it('should support generic types for select', async () => {
      ...
      const promptPromise = select<Option>({
        message: 'Choose:',
      ...
      // TypeScript should infer Promise<Option>
      const typeCheck: Promise<Option> = promptPromise;
      expect(typeCheck).toBeDefined();
  ...
  describe('theme integration', () => {
    it('should support custom themes', async () => {
      // Just verify that the API accepts theme option
      const promptPromise = text({
        message: 'Test:',
        theme: {
          symbols: {
