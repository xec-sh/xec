...
import { 
  ...
  type MouseEvent,
  withMouseSupport
} from '../../../src/utils/mouse-support.js';
...
describe('MouseSupport', () => {
  ...
  describe('region management', () => {
    it('should initialize with regions', () => {
      const regions: Region[] = [
        ...
        { x: 0, y: 60, width: 100, height: 10 } // No ID
      ];
      
      const mouse = new MouseSupport({ regions });
      
      // Check internal state - regions with IDs should be stored
      expect(mouse['regions'].has('header')).toBe(true);
      expect(mouse['regions'].has('body')).toBe(true);
      expect(mouse['regions'].size).toBe(2); // Only regions with IDs
    });
    ...
    it('should add regions', () => {
      ...
      // Region should be stored internally
  ...
  describe('click handling', () => {
    it('should handle click events', () => {
      ...
      expect(handled).toBe(false); // No region at position
    ...
    it('should detect clicks on regions', () => {
      ...
      expect(regionClickHandler).toHaveBeenCalledWith(
        expect.objectContaining({
    ...
    it('should detect double clicks', async () => {
      ...
      expect(clickHandler).toHaveBeenLastCalledWith(
        expect.objectContaining({
          isDoubleClick: true,
        })
      );
  ...
  describe('scroll handling', () => {
    ...
    it('should handle scroll on regions', () => {
      ...
      expect(regionScrollHandler).toHaveBeenCalledWith(
        expect.objectContaining({
  ...
  describe('static helpers', () => {
    ...
    it('should parse SGR mouse sequences', () => {
      ...
      expect(event).toEqual({
        type: 'click',
        position: { x: 9, y: 19 }, // 1-indexed to 0-indexed
        button: 'left',
      });
    });
  ...
  describe('edge cases', () => {
    it('should clear hoveredRegion when removing hovered region', () => {
      ...
      // The hoveredRegion should be set (internal state)
      // Now remove the region
      mouse.removeRegion('hoverable');
      
      // Verify the region is removed
...
describe('TerminalMouse', () => {
  it('should enable mouse mode', () => {
    ...
    expect(writer).toHaveBeenCalledWith('\x1b[?1000h'); // Enable normal mode
    expect(writer).toHaveBeenCalledWith('\x1b[?1006h'); // Enable SGR mode
    expect(writer).toHaveBeenCalledWith('\x1b[?1004h'); // Enable focus events
  ...
  it('should disable mouse mode', () => {
    ...
    expect(writer).toHaveBeenCalledWith('\x1b[?1000l'); // Disable normal mode
    expect(writer).toHaveBeenCalledWith('\x1b[?1006l'); // Disable SGR mode
    expect(writer).toHaveBeenCalledWith('\x1b[?1004l'); // Disable focus events
  ...
  it('should switch modes', () => {
    ...
    // Should disable old mode first
    expect(writer).toHaveBeenCalledWith('\x1b[?1000l');
    
    // Then enable new mode
...
describe('withMouseSupport mixin', () => {
  ...
  it('should handle mouse events', () => {
    ...
    expect(handled).toBe(false); // No regions
  });
