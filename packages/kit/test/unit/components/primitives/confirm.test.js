...
describe('ConfirmPrompt', () => {
    ...
    describe('non-TTY mode', () => {
        it('should return initial value in non-TTY', async () => {
            const result = await testNonTTYPrompt(ConfirmPrompt, {
                message: 'Continue?',
                initialValue: true
            }, true);
        ...
        it('should return false when no initial value', async () => {
            const result = await testNonTTYPrompt(ConfirmPrompt, { message: 'Continue?' }, false);
            expect(result).toBe(false);
        });
    });
    describe('TTY mode - input handling', () => {
        it('should accept y for yes', async () => {
            const result = await testPrompt(ConfirmPrompt, { message: 'Continue?' }, async ({ sendKey }) => {
                sendKey('y');
            });
            expect(result).toBe(true);
        });
        ...
        it('should toggle with arrow keys', async () => {
            const result = await testPrompt(ConfirmPrompt, { message: 'Continue?', defaultValue: false }, async ({ sendKey }) => {
                sendKey({ name: 'right' });
                sendKey({ name: 'return' });
            });
            expect(result).toBe(true);
        ...
        it('should toggle with left arrow', async () => {
            const result = await testPrompt(ConfirmPrompt, { message: 'Continue?', defaultValue: true }, async ({ sendKey }) => {
                sendKey({ name: 'left' });
                sendKey({ name: 'return' });
            });
            expect(result).toBe(false);
    ...
    describe('rendering', () => {
        it('should show yes/no options', async () => {
            await testPrompt(ConfirmPrompt, { message: 'Continue?' }, async ({ mockStreams, sendKey }) => {
        ...
        it('should highlight current selection', async () => {
            await testPrompt(ConfirmPrompt, { message: 'Continue?', defaultValue: true }, async ({ mockStreams, sendKey }) => {
                await new Promise(resolve => setTimeout(resolve, 10));
                const output = mockStreams.stdout.write.mock.calls.map(c => c[0]).join('');
                expect(output).toContain('Continue?');
                sendKey({ name: 'escape' });
        ...
        it('should show custom labels', async () => {
            await testPrompt(ConfirmPrompt, {
                message: 'Delete file?',
                yesLabel: 'Delete',
                noLabel: 'Keep'
    ...
    describe('validation', () => {
        it('should validate response', async () => {
            ...
            const result = await testPrompt(ConfirmPrompt, { message: 'Agree?', validate }, async ({ sendKey, waitForRender }) => {
                sendKey('n');
                sendKey({ name: 'return' });
                await waitForRender();
                sendKey('y');
    ...
    describe('edge cases', () => {
        ...
        it.skip('should handle Tab key as toggle', async () => {
            const result = await testPrompt(ConfirmPrompt, { message: 'Continue?', defaultValue: false }, async ({ sendKey, waitForRender }) => {
                sendKey({ name: 'tab' });
                await waitForRender();
                sendKey({ name: 'return' });
            });
