...
describe('TextPrompt', () => {
  describe('initialization', () => {
    ...
    it('should accept default value', () => {
      ...
      expect((prompt as any).value).toBe('Alice');
    });
  ...
  describe('TTY mode - input handling', () => {
    ...
    it('should handle backspace', async () => {
      const result = await testPrompt(
        TextPrompt,
        { message: 'Enter:', defaultValue: 'test' },
        async ({ sendKey }) => {
  ...
  describe('rendering', () => {
    it('should render placeholder when empty', async () => {
      await testPrompt(
        TextPrompt,
        { message: 'Name:', placeholder: 'John Doe' },
    ...
    it.skip('should not show placeholder when value exists', async () => {
      await testPrompt(
        TextPrompt,
        { message: 'Name:', placeholder: 'John Doe' },
        async ({ mockStreams, sendKey, waitForRender }) => {
          // Send characters individually with waits
          for (const char of 'Alice') {
            sendKey(char);
            await waitForRender();
          }
          await new Promise(resolve => setTimeout(resolve, 100)); // Give more time for final render
          // Get all writes and filter only string content
          const writes = mockStreams.stdout.write.mock.calls
            .map(c => c[0])
            .filter(c => typeof c === 'string');
          
          // Get the last complete render (should contain the full prompt)
          const lastCompleteRender = writes.reverse().find(w => w.includes('Name:')) || '';
          
          // Remove ANSI codes for cleaner comparison
    ...
    it('should show cursor position', async () => {
      await testPrompt(
        ...
        async ({ mockStreams, sendKey }) => {
          sendKey({ name: 'home' });
          await new Promise(resolve => setTimeout(resolve, 10));
          // Should show cursor at beginning
          sendKey({ name: 'escape' });
        }
      );
    });
  ...
  describe('password masking', () => {
    ...
    it.skip('should use custom mask function', async () => {
      const mask = vi.fn((char: string) => char === 'a' ? 'A' : '*');
