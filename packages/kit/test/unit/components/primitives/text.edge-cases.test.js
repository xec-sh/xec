...
describe('TextPrompt - Edge Cases', () => {
    describe('empty and whitespace handling', () => {
        ...
        it('should reject empty input when required', async () => {
            const result = await testPrompt(TextPrompt, { message: 'Enter:', required: true }, async ({ sendKey, getLastRender, waitForRender }) => {
                sendKey({ name: 'return' });
                await waitForRender();
    ...
    describe('extremely long input', () => {
        ...
        it('should handle input at maxLength boundary', async () => {
            const result = await testPrompt(TextPrompt, { message: 'Enter:', maxLength: 5 }, async ({ sendKey }) => {
                sendKey('12345');
                sendKey('6');
                sendKey({ name: 'return' });
            });
        ...
        it('should prevent input beyond maxLength', async () => {
            const result = await testPrompt(TextPrompt, { message: 'Enter:', maxLength: 3 }, async ({ sendKey }) => {
                sendKey('abcdef');
                sendKey({ name: 'return' });
            });
    ...
    describe('validation edge cases', () => {
        it('should handle validator that always rejects', async () => {
            const result = await testPrompt(TextPrompt, {
                message: 'Enter:',
    ...
    describe('multiline edge cases', () => {
        it('should handle multiline with empty lines', async () => {
            const result = await testPrompt(TextPrompt, { message: 'Enter:', multiline: true }, async ({ sendKey }) => {
                sendKey('line1');
                sendKey({ name: 'return' });
                sendKey({ name: 'return' });
                sendKey('line3');
    ...
    describe('placeholder behavior', () => {
        it('should show placeholder for empty input', async () => {
            await testPrompt(TextPrompt, { message: 'Enter:', placeholder: 'Type here...' }, async ({ getLastRender, sendKey }) => {
                const output = getLastRender();
        ...
        it('should hide placeholder when typing', async () => {
            await testPrompt(TextPrompt, { message: 'Enter:', placeholder: 'Type here...' }, async ({ sendKey, getLastRender, waitForRender }) => {
                sendKey('a');
                await waitForRender();
                const output = getLastRender();
    ...
    describe('configuration edge cases', () => {
        it('should handle conflicting minLength and maxLength', async () => {
            ...
            await testPrompt(TextPrompt, options, async ({ sendKey }) => {
                sendKey('test');
                sendKey({ name: 'return' });
                sendKey({ name: 'escape' });
            });
...
//# sourceMappingURL=text.edge-cases.test.js.map