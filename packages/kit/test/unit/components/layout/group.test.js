...
describe('GroupPrompt', () => {
    ...
    describe('sequential execution', () => {
        it('should run prompts in order', async () => {
            ...
            const prompt = new GroupPrompt({
                message: 'Test Group',
                prompts: [
                    { name: 'field1', prompt: mockTextPrompt1 },
                    { name: 'field2', prompt: mockTextPrompt2 }
                ]
            });
    ...
    describe('conditional prompts', () => {
        it('should skip prompts based on condition', async () => {
            ...
            const prompt = new GroupPrompt({
                message: 'Test Group',
                prompts: [
                    { name: 'skipNext', prompt: mockPrompt1 },
                    {
                        name: 'conditional',
                        prompt: mockPrompt2,
        ...
        it('should include prompts when condition is met', async () => {
            ...
            const prompt = new GroupPrompt({
                message: 'Test Group',
                prompts: [
                    { name: 'includeNext', prompt: mockPrompt1 },
                    {
                        name: 'conditional',
                        prompt: mockPrompt2,
    ...
    describe('helper function', () => {
        ...
        it('should support conditional functions', () => {
            const prompts = {
                ...
                email: (results) => results.hasEmail ?
    ...
    describe('result formatting', () => {
        it('should format boolean values', () => {
            ...
            const formatted = prompt.formatResult('test', true);
            expect(formatted).toBe('Yes');
            const formattedFalse = prompt.formatResult('test', false);
        ...
        it('should format array values', () => {
            ...
            const formatted = prompt.formatResult('test', ['a', 'b', 'c']);
        ...
        it('should format other values as strings', () => {
            ...
            const formatted = prompt.formatResult('test', 42);
    ...
    describe('final render', () => {
        it('should show summary of results', async () => {
            ...
            const prompt = new GroupPrompt({
                ...
                prompts: [
                    { name: 'name', prompt: mockPrompt1 },
                    { name: 'newsletter', prompt: mockPrompt2 }
                ]
            });
            await prompt.prompt();
