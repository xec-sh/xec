...
describe('ColumnsPrompt', () => {
  ...
  describe('pane sizing', () => {
    it('should calculate sizes based on percentages', () => {
      ...
      (prompt as any).calculatePaneSizes();
      const sizes = (prompt as any).paneSizes;
      
      expect(sizes).toHaveLength(2);
      // Sizes should be proportional (exact values depend on terminal width)
    ...
    it('should respect min/max constraints', () => {
      ...
      expect(sizes[0]).toBe(20); // Should be adjusted to minWidth
    });
  });
    ...
    it('should switch focus between panes', async () => {
      await testPrompt(
        ...
        async ({ prompt, sendKey, waitForRender }) => {
          ...
          expect((prompt as any).state.getState().focusedPaneIndex).toBe(0);
          
          sendKey('tab');
          await new Promise(resolve => setTimeout(resolve, 50));
          expect((prompt as any).state.getState().focusedPaneIndex).toBe(1);
          
          sendKey('tab');
          await new Promise(resolve => setTimeout(resolve, 50));
          expect((prompt as any).state.getState().focusedPaneIndex).toBe(2);
          
          sendKey('tab'); // Should wrap around
          await new Promise(resolve => setTimeout(resolve, 50));
          expect((prompt as any).state.getState().focusedPaneIndex).toBe(0);
    ...
    it('should handle scrolling in scrollable panes', async () => {
      ...
      await testPrompt(
        ...
        async ({ prompt, sendKey, waitForRender }) => {
          ...
          expect((prompt as any).state.getState().scrollOffsets[0]).toBe(0);
          
          sendKey('down');
          await new Promise(resolve => setTimeout(resolve, 50));
          expect((prompt as any).state.getState().scrollOffsets[0]).toBe(1);
          ...
          sendKey('pagedown');
          await new Promise(resolve => setTimeout(resolve, 50));
          expect((prompt as any).state.getState().scrollOffsets[0]).toBeGreaterThan(0);
    ...
    it('should toggle resize mode', async () => {
      await testPrompt(
        ...
        async ({ prompt, sendKey, waitForRender }) => {
          ...
          expect((prompt as any).state.getState().isResizing).toBe(false);
          
          sendKey({ name: 'r', ctrl: true });
          await new Promise(resolve => setTimeout(resolve, 50));
          expect((prompt as any).state.getState().isResizing).toBe(true);
    ...
    it('should adjust pane sizes in resize mode', async () => {
      await testPrompt(
        ...
        async ({ prompt, sendKey, waitForRender }) => {
          ...
          const initialSizes = [...(prompt as any).paneSizes];
          
          sendKey({ name: 'r', ctrl: true }); // Enter resize mode
          await waitForRender();
          
          sendKey('right');
          await new Promise(resolve => setTimeout(resolve, 50));
          const newSizes = (prompt as any).paneSizes;
  ...
  describe('exit behavior', () => {
    it.skip('should exit on return', async () => {
      // TODO: Fix return/enter key handling in test environment
      await testPrompt(
        ColumnsPrompt,
        {
          message: 'Test Columns',
  ...
  describe('utility methods', () => {
    it('should center text', () => {
      ...
      const centered = (prompt as any).centerText('Hello', 10);
    ...
    it('should truncate long text', () => {
      ...
      const truncated = (prompt as any).truncateText('This is a very long text', 10);
