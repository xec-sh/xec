...
describe('WizardPrompt', () => {
  ...
  describe('rendering', () => {
    ...
    it('should show progress indicator', async () => {
      ...
      const rendered = prompt.render();
      expect(rendered).toContain('○'); // Empty dots
      expect(rendered).toContain('(1/3)'); // Progress text
    });
  ...
  describe('page navigation', () => {
    ...
    it('should handle page skip conditions', async () => {
      ...
      const prompt = new WizardPrompt({
        ...
        pages: [
          {
            id: 'shouldSkip',
            render: () => mockPrompt1 as any
          },
          {
            id: 'skippedPage',
            render: () => mockPrompt2 as any,
            skip: (context) => context.shouldSkip === false
          },
          {
            id: 'finalPage',
            render: () => mockPrompt3 as any
          }
        ]
      });
    ...
    it('should support string content pages', async () => {
      ...
      // In non-interactive mode, panels will return cancel symbol
      const result = await prompt.prompt();
  ...
  describe('callbacks', () => {
    it('should call onPageChange', async () => {
      const pageChanges: Array<{ from: string; to: string }> = [];
      ...
      const prompt = new WizardPrompt({
        message: 'Test Wizard',
        pages: [
          { id: 'page1', render: () => mockPrompt1 as any },
          { id: 'page2', render: () => mockPrompt2 as any }
        ],
    ...
    it('should call onComplete', async () => {
      ...
      const prompt = new WizardPrompt({
        message: 'Test Wizard',
        pages: [
          { id: 'page1', render: () => mockPrompt as any }
        ],
  ...
  describe('progress rendering', () => {
    it('should render progress dots', () => {
      ...
      const progress = (prompt as any).renderProgress(2, 3);
      expect(progress).toContain('●'); // Filled dots
      expect(progress).toContain('○'); // Empty dots
  ...
  describe('final render', () => {
    it('should show completion summary', async () => {
      ...
      const prompt = new WizardPrompt({
        ...
        title: 'User Setup',
        pages: [
          { id: 'name', title: 'Name', render: () => mockPrompt1 as any },
          { id: 'role', title: 'Role', render: () => mockPrompt2 as any }
        ]
      });
    ...
    it('should format different value types', () => {
      ...
      const formatMethod = (prompt as any).formatPageResult;
