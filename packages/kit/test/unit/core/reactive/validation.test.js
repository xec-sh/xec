...
describe('ReactiveValidator', () => {
    ...
    beforeEach(() => {
        ...
        validator = new ReactiveValidator(state, {
            ...
            terms: validators.custom(value => value === true, 'You must accept the terms and conditions'),
        });
    });
    ...
    describe('computed validation state', () => {
        it('should create computed validation values', async () => {
            await validator.validateAll();
            const formValid = validator.computed(getValidation => ['username', 'email', 'password', 'terms'].every(field => getValidation(field).valid));
    ...
    describe('async validation', () => {
        it('should handle async validation rules', async () => {
            const asyncValidator = new ReactiveValidator(state, {
                username: validators.compose(validators.required(), validators.custom(async (value) => {
                    await new Promise(resolve => setTimeout(resolve, 10));
                    return value !== 'taken';
    ...
    describe('validators utility', () => {
        it('should compose multiple validators', async () => {
            const strongPassword = validators.compose(validators.required('Password is required'), validators.minLength(8, 'At least 8 characters'), validators.pattern(/[A-Z]/, 'Must contain uppercase letter'), validators.pattern(/[0-9]/, 'Must contain number'), validators.pattern(/[!@#$%^&*]/, 'Must contain special character'));
