...
describe('StreamHandler', () => {
  ...
  describe('shared mode', () => {
    ...
    it('should handle reference counting', () => {
      ...
      // First acquire should start the stream
      handler.acquire();
      expect(handler.getRefCount()).toBe(1);
      expect(handler.isActive()).toBe(true);
      
      // Second acquire should increment ref count
      handler.acquire();
      ...
      // Second release should stop the stream
      handler.release();
      expect(handler.getRefCount()).toBe(0);
      expect(handler.isActive()).toBe(false);
    });
    ...
    it('should not stop shared stream if still referenced', () => {
      ...
      // Try to stop directly - should not stop
      handler.stop();
      expect(handler.isActive()).toBe(true);
...
describe('StreamHandlerFactory', () => {
  ...
  it('should provide statistics', () => {
    ...
    shared.acquire(); // Use acquire for shared streams
    named1.acquire();
