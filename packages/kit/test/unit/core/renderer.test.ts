...
describe('Renderer', () => {
  ...
  describe('basic rendering', () => {
    it('should create renderer with theme', () => {
      ...
      expect((renderer as any).theme).toStrictEqual(theme);
    });
    ...
    it('should render simple content', () => {
      ...
      vi.runAllTimers(); // Wait for throttled render
    ...
    it('should clear previous content', async () => {
      ...
      // Advance time to bypass throttling
      vi.advanceTimersByTime(20);
      
      renderer.render('New content');
      vi.runAllTimers(); // Wait for throttled render
      
      // Should clear previous lines and write new content
  ...
  describe('frame management', () => {
    it('should track previous frame', async () => {
      ...
      vi.runAllTimers(); // Wait for throttled render
      expect((renderer as any).previousFrame).toBe('Frame 1');
      ...
      vi.runAllTimers(); // Wait for throttled render
      expect((renderer as any).previousFrame).toBe('Frame 2');
    });
  ...
  describe('region updates', () => {
    it('should update specific region', () => {
      ...
      (mockStream as any).cursorTo = mockCursorTo;
  ...
  describe('text measurement', () => {
    it('should measure text dimensions', () => {
      ...
      expect(dims.width).toBe(5); // "Hello" and "World" are both 5 chars
    ...
    it('should handle empty text', () => {
      ...
      expect(dims.height).toBe(1); // Empty string still has 1 line
    });
    ...
    it('should find longest line', () => {
      ...
      expect(dims.width).toBe(21); // "This is a longer line"
      expect(dims.height).toBe(3);
    });
  });
  ...
  describe('theme integration', () => {
    it('should use theme for rendering', () => {
      ...
      expect((renderer as any).theme).toStrictEqual(theme);
      expect((renderer as any).theme.symbols.success).toBeDefined();
      expect((renderer as any).theme.formatters.error).toBeDefined();
    });
  });
  ...
  describe('node rendering', () => {
    it('should render text nodes', () => {
      const renderer = new Renderer({ theme: createDefaultTheme(), stream: mockStream });
      const renderNode = (renderer as any).renderNode.bind(renderer);
    ...
    it('should render line nodes', () => {
      ...
      const output = renderNode(node);
      expect(output).toMatch(/─{80}/); // 80 horizontal line chars
    });
  ...
  describe('style application', () => {
    ...
    it('should apply background color', () => {
      ...
      expect(styled).toContain('\x1B[43m'); // Yellow background
      expect(styled).toContain('Text');
      expect(styled).toContain('\x1B[49m'); // Reset
    });
    ...
    it('should handle unknown colors', () => {
      ...
      expect(styled).toBe('Text\x1B[39m'); // Just reset code
    });
    ...
    it('should handle style attributes that are not implemented', () => {
      ...
      // These styles are commented out in the implementation
  ...
  describe('ANSI stripping', () => {
    it('should strip ANSI codes when measuring', () => {
      ...
      expect(dims.width).toBe(8); // "Red Text" without ANSI codes
    });
  });
});