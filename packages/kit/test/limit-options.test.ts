...
import { limitOptions, type LimitOptionsParams } from '../src/utilities/limit-options.js';
...
describe('limitOptions', () => {
  ...
  test('clamps to 5 rows minimum', async () => {
    ...
    options.maxItems = 3;
    const result = limitOptions(options);
    expect(result).toEqual(['Item 1', 'Item 2', 'Item 3', 'Item 4', prism.dim('...')]);
  });
  ...
  test('returns sliding window near end of list', async () => {
    ...
    options.cursor = 8;
    const result = limitOptions(options);
    expect(result).toEqual([prism.dim('...'), 'Item 7', 'Item 8', 'Item 9', 'Item 10']);
  });
  ...
  test('if items exceed output height, clamp to fit', async () => {
    ...
    options.maxItems = 10;
    const result = limitOptions(options);
    expect(result).toEqual(['Item 1', 'Item 2', prism.dim('...')]);
  });
  ...
  test('style option receives correct cursor index', async () => {
    ...
    options.maxItems = 5;
    options.cursor = 1;
    options.style = (option, isSelected) => (isSelected ? `-- ${option.value} --` : option.value);
    const result = limitOptions(options);
    expect(result).toEqual(['Item 1', '-- Item 2 --', 'Item 3']);
  });
