...
describe.each(['true', 'false'])('selectKey (isCI = %s)', (isCI) => {
  ...
  // caseSensitive compares the char exactly as delivered (upstream #534); the
  // old keyInfo.shift reconstruction broke under Caps Lock and pasted input.
  test('caseSensitive: true makes options case-sensitive', async () => {
    ...
    // lowercase 'a' must not match the uppercase-only option 'A'
    input.emit('keypress', 'a', { name: 'a' });
