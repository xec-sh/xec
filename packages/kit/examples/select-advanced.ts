...
async function fetchUsers(): Promise<ApiUser[]> {
	...
	return [
		{ id: 1, name: 'Leanne Graham', email: 'leanne@example.com', company: { name: 'Romaguera-Crona' } },
		{ id: 2, name: 'Ervin Howell', email: 'ervin@example.com', company: { name: 'Deckow-Crist' } },
		{ id: 3, name: 'Clementine Bauch', email: 'clementine@example.com', company: { name: 'Romaguera-Jacobson' } },
		{ id: 4, name: 'Patricia Lebsack', email: 'patricia@example.com', company: { name: 'Robel-Corkery' } },
		{ id: 5, name: 'Chelsey Dietrich', email: 'chelsey@example.com', company: { name: 'Keebler LLC' } },
	];
}
...
async function fetchRepos(): Promise<GitHubRepo[]> {
	...
	return [
		{ name: 'react', full_name: 'facebook/react', description: 'A declarative UI library', stargazers_count: 200000, language: 'JavaScript' },
		{ name: 'vue', full_name: 'vuejs/vue', description: 'Progressive framework', stargazers_count: 180000, language: 'JavaScript' },
		{ name: 'angular', full_name: 'angular/angular', description: 'Platform for web apps', stargazers_count: 85000, language: 'TypeScript' },
		{ name: 'svelte', full_name: 'sveltejs/svelte', description: 'Compile-time framework', stargazers_count: 70000, language: 'JavaScript' },
		{ name: 'solid', full_name: 'solidjs/solid', description: 'Reactive UI library', stargazers_count: 28000, language: 'TypeScript' },
		{ name: 'qwik', full_name: 'BuilderIO/qwik', description: 'Resumable framework', stargazers_count: 18000, language: 'TypeScript' },
		{ name: 'alpine', full_name: 'alpinejs/alpine', description: 'Rugged framework', stargazers_count: 25000, language: 'JavaScript' },
		{ name: 'lit', full_name: 'lit/lit', description: 'Web Components library', stargazers_count: 15000, language: 'TypeScript' },
	];
}
...
async function main() {
	...
	const repoOptions = category === 'js' ? jsRepos :
	                   category === 'ts' ? tsRepos :
	                   repos;
	...
	const instanceTypes = environment === 'prod'
		? [
	...
	const plans: PricingPlan[] = [
		...
		{ id: 'starter', name: 'Starter', price: 9, features: ['All Free features', '50 projects', 'Email support'] },
		{ id: 'pro', name: 'Pro', price: 29, features: ['All Starter features', 'Unlimited projects', 'Priority support', 'Advanced analytics'], recommended: true },
		{ id: 'enterprise', name: 'Enterprise', price: 99, features: ['All Pro features', 'Custom integrations', 'Dedicated support', 'SLA'] },
	];
	...
	if (!isCancel(searchCategory)) {
		const filtered = searchCategory === 'frontend' 
