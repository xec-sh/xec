...
async function main() {
	...
	const choice = await p.select({
		...
		options: [
			{ value: 'source-destination', label: '📋 Source & Destination Selection', hint: 'Copy/move operations' },
			{ value: 'project-setup', label: '🏗️ Project Setup Wizard', hint: 'Multi-step path selection' },
			{ value: 'backup-selection', label: '💾 Backup File Selection', hint: 'Filter by date and type' },
			{ value: 'permission-check', label: '🔒 Permission-aware Selection', hint: 'Check read/write access' },
			{ value: 'relative-paths', label: '📍 Relative Path Resolution', hint: 'Work with relative paths' },
...
async function backupSelectionExample() {
	...
	const backupSource = await p.path({
		...
		validate: (value) => {
			...
			if (stats.size > 1024 * 1024 * 100) { // 100MB
...
async function permissionCheckExample() {
	...
	const selectedFile = await p.path({
		...
		validate: (value) => {
			...
			try {
				...
				const perms = [];
				try { accessSync(value, constants.R_OK); perms.push('R'); } catch {}
				try { accessSync(value, constants.W_OK); perms.push('W'); } catch {}
				try { accessSync(value, constants.X_OK); perms.push('X'); } catch {}
	...
	const perms = [];
	try { accessSync(selectedFile, constants.R_OK); perms.push('READ'); } catch {}
	try { accessSync(selectedFile, constants.W_OK); perms.push('WRITE'); } catch {}
	try { accessSync(selectedFile, constants.X_OK); perms.push('EXECUTE'); } catch {}
