...
async function main() {
    ...
    const subtotal = computed(cart, get => get('items').reduce((sum, item) => sum + item.price * item.quantity, 0));
    const discountAmount = computed(cart, get => subtotal() * get('discount'));
    const taxAmount = computed(cart, get => (subtotal() - discountAmount()) * get('taxRate'));
    const total = computed(cart, get => subtotal() - discountAmount() + taxAmount());
