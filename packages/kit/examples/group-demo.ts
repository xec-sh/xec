...
async function main() {
  ...
  const projectSetup = await group(
    {
      ...
      database: ({ results }) => {
        if (!results.features || !results.features.includes('db')) return;
      ...
      installation: async ({ results }) => {
        ...
        if (results.features?.includes('tests')) {
  ...
  if (projectSetup.projectName) {
    ...
    note(
      ...
      `Features: ${projectSetup.features?.join(', ') || 'None'}\n` +
  ...
  const complexForm = await group(
    {
      ...
      priority: ({ results }) => {
        if (!results.allocations || results.allocations.length === 0) return;
        
        return select({
          message: 'Which allocation has the highest priority?',
          options: results.allocations.map(alloc => ({
            value: alloc,
      ...
      confirm: ({ results }) => {
        const summary = `
          ...
          Allocations: ${results.allocations?.join(', ') || 'None'}
