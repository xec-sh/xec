...
export const stream = {
  message: async (
    ...
    { symbol = prism.gray(S_BAR) }: LogMessageOptions = {}
  ) => {
    process.stdout.write(`${prism.gray(S_BAR)}\n${symbol}  `);
    let lineWidth = 3;
  ...
  step: (iterable: Iterable<string> | AsyncIterable<string>) =>
    stream.message(iterable, { symbol: prism.green(S_STEP_SUBMIT) }),
  warn: (iterable: Iterable<string> | AsyncIterable<string>) =>
    stream.message(iterable, { symbol: prism.yellow(S_WARN) }),
  /** alias for `log.warn()`. */
  ...
  error: (iterable: Iterable<string> | AsyncIterable<string>) =>
    stream.message(iterable, { symbol: prism.red(S_ERROR) }),
};
