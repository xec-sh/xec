...
export interface DateFormatConfig {
  ...
  /** Labels shown when segment is blank (e.g. yyyy, mm, dd). */
...
/** Convert Date directly to segment values */
...
/** Build a Date from segment values using UTC midnight */
...
function getSegmentValidationMessage(parts: DateParts, seg: SegmentConfig): string | undefined {
  ...
  if (seg.type === 'day') {
    ...
    if (day > daysInMonth(year, month)) {
      const monthName =
        month >= 1 && month <= 12 ? (settings.date.monthNames[month - 1] ?? 'this month') : 'this month';
