 ...
 * a dependency` — so the whole package fails to load before a single line of
 * ours runs. Node and Bun accept both spellings, which is why nothing else
...
describe('the built package imports Node builtins portably', () => {
  ...
  const BUILTINS = [
    ...
    'https', 'net', 'os', 'path', 'perf_hooks', 'readline', 'repl', 'stream',
