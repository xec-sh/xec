...
describe('Kubernetes API', () => {
    ...
    describe('K8sPod', () => {
        ...
        describe('port forwarding', () => {
            ...
            beforeEach(() => {
                mockAdapter.portForward.mockReturnValue(mockPortForward);
            });
            ...
            it('should create port forward with dynamic local port', async () => {
                ...
                expect(mockAdapter.portForward).toHaveBeenCalledWith('test-pod', 0, 3000, { namespace: 'test-namespace', dynamicLocalPort: true });
                expect(mockPortForward.open).toHaveBeenCalled();
                expect(forward).toBe(mockPortForward);
            });
        });
        describe('logs', () => {
            it('should get logs', async () => {
                ...
                expect(mockAdapter.executeKubectl).toHaveBeenCalledWith(['logs', '-n', 'test-namespace', '--tail', '10', '--timestamps', 'test-pod'], { throwOnNonZeroExit: true });
            ...
            it('should get logs from specific container', async () => {
                ...
                expect(mockAdapter.executeKubectl).toHaveBeenCalledWith(['logs', '-n', 'test-namespace', '-c', 'nginx', '--previous', 'test-pod'], { throwOnNonZeroExit: true });
        ...
        describe('file operations', () => {
            ...
            it('should copy file from specific container', async () => {
                ...
                expect(mockAdapter.copyFiles).toHaveBeenCalledWith('test-pod:/var/log/nginx/access.log -c nginx', './nginx-access.log', {
                    namespace: 'test-namespace',
                    container: 'nginx',
                    direction: 'from'
                });
    ...
    describe('Edge cases', () => {
        ...
        it('should handle logs options correctly', async () => {
            ...
            const pod = context.pod('test-pod');
            await pod.logs();
            expect(mockAdapter.executeKubectl).toHaveBeenCalledWith(['logs', '-n', 'default', 'test-pod'], { throwOnNonZeroExit: true });
            await pod.logs({
                container: 'app',
                tail: 100,
                previous: true,
            ...
            expect(mockAdapter.executeKubectl).toHaveBeenCalledWith(['logs', '-n', 'default', '-c', 'app', '--tail', '100', '--previous', '--timestamps', 'test-pod'], { throwOnNonZeroExit: true });
        });
...
//# sourceMappingURL=kubernetes-api.test.js.map