...
describe('SSHAdapter - Unit Tests', () => {
    ...
    describe('SFTP Configuration Validation', () => {
        it('should throw error when SFTP is disabled for upload', async () => {
            ...
            await expect(adapter.uploadFile('/local/path/file.txt', '/remote/path/file.txt', { type: 'ssh', host: 'test-host', username: 'test' })).rejects.toThrow('SFTP is disabled');
        });
        it('should throw error when SFTP is disabled for download', async () => {
            adapter = new SSHAdapter({
                sftp: { enabled: false, concurrency: 5 }
            });
            await expect(adapter.downloadFile('/remote/path/file.txt', '/local/path/file.txt', { type: 'ssh', host: 'test-host', username: 'test' })).rejects.toThrow('SFTP is disabled');
        });
        it('should throw error when SFTP is disabled for directory upload', async () => {
            adapter = new SSHAdapter({
                sftp: { enabled: false, concurrency: 5 }
            });
            await expect(adapter.uploadDirectory('/local/dir', '/remote/dir', { type: 'ssh', host: 'test-host', username: 'test' })).rejects.toThrow('SFTP is disabled');
        });
    });
    ...
    describe('Input Validation', () => {
        it('should handle various SSH options configurations', () => {
            adapter = new SSHAdapter();
            const validConfigs = [
                { type: 'ssh', host: 'localhost', username: 'user' },
                { type: 'ssh', host: '192.168.1.1', username: 'admin', port: 2222 },
                { type: 'ssh', host: 'example.com', username: 'user', password: 'secret' },
                { type: 'ssh', host: 'server.com', username: 'dev', privateKey: 'key-content' },
                { type: 'ssh', host: 'test.local', username: 'test', privateKey: 'key', passphrase: 'phrase' }
            ];
...
//# sourceMappingURL=ssh-adapter-unit.test.js.map