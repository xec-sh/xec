...
class TestAdapter extends BaseAdapter {
    ...
    async testCreateResult(stdout, stderr, exitCode, signal, command, startTime, endTime, context) {
    ...
    testBuildCommandString(command) {
        return this.buildCommandString(command);
    }
    testCreateCombinedEnv(baseEnv, commandEnv) {
        return this.createCombinedEnv(baseEnv, commandEnv);
    }
    testMergeCommand(command) {
        return this.mergeCommand(command);
    }
    async testHandleAbortSignal(signal, cleanup) {
        return this.handleAbortSignal(signal, cleanup);
    }
    testCreateStreamHandler(options) {
        return this.createStreamHandler(options);
    }
    testEmitAdapterEvent(event, data) {
        return this.emitAdapterEvent(event, data);
    }
    testCreateProgressReporter(command) {
        return this.createProgressReporter(command);
    }
    async testHandleTimeout(promise, timeout, command, cleanup) {
        return this.handleTimeout(promise, timeout, command, cleanup);
    }
    testCreateResultSync(stdout, stderr, exitCode, signal, command, startTime, endTime, context) {
...
-----END RSA PRIVATE KEY-----`;
      ...
      `;
        ...
        test('should mask stdout and stderr in execution results', async () => {
            ...
            const result = await adapter.testCreateResult('Command output with api_key=secret123', 'Error: password=mypassword failed', 1, undefined, 'echo $API_KEY', Date.now(), Date.now());
        ...
        test('should handle empty and null inputs gracefully', () => {
            ...
            expect(adapter.testMaskSensitiveData(null)).toBe(null);
            expect(adapter.testMaskSensitiveData(undefined)).toBe(undefined);
        });
        test('should mask data in streaming handler', () => {
            const adapter = new TestAdapter();
            let capturedData = '';
            const handler = adapter.createStreamHandler({
                onData: (chunk) => {
                    capturedData += chunk;
                }
            });
    ...
    describe('Configuration', () => {
        test('should have sensible defaults', () => {
            ...
            expect(config.sensitiveDataMasking.patterns).toHaveLength(14);
        });
    ...
    describe('shouldThrowOnNonZeroExit', () => {
        ...
        test('should respect command.nothrow when set', () => {
            const adapter = new TestAdapter({ throwOnNonZeroExit: true });
            const nothrowCommand = { command: 'test', nothrow: true };
            expect(adapter.testShouldThrowOnNonZeroExit(nothrowCommand, 1)).toBe(false);
            const throwCommand = { command: 'test', nothrow: false };
    ...
    describe('buildCommandString', () => {
        ...
        test('should handle empty args array', () => {
            const adapter = new TestAdapter();
            const command = { command: 'echo', args: [] };
            expect(adapter.testBuildCommandString(command)).toBe('echo');
        });
    });
    describe('createCombinedEnv', () => {
        test('should combine base and command env', () => {
            ...
            expect(combined['COMMON']).toBe('command');
        });
    ...
    describe('mergeCommand', () => {
        test('should merge command with adapter defaults', () => {
            ...
            expect(merged.cwd).toBe('/command');
            expect(merged.env).toEqual({ BASE: 'base', COMMAND: 'command' });
    ...
    describe('Timeout Handling', () => {
        ...
        test('should timeout when promise takes too long', async () => {
            ...
            await expect(adapter.testHandleTimeout(promise, 100, 'test command', cleanup)).rejects.toThrow('Command timed out');
            expect(cleanup).toHaveBeenCalled();
        });
    ...
    describe('createResultSync', () => {
        test('should create result with masking', () => {
            ...
            const result = adapter.testCreateResultSync('Output with password=secret123', 'Error with api_key=abc123', 0, undefined, 'echo password=secret123', startTime, endTime);
        ...
        test('should not throw with nothrow command', () => {
            ...
            const startTime = Date.now();
            const endTime = startTime + 1000;
            const result = adapter.testCreateResultSync('stdout', 'stderr', 1, undefined, 'failing command', startTime, endTime, { originalCommand: { command: 'test', nothrow: true } });
        ...
        test('should include context in result', () => {
            ...
            const startTime = Date.now();
            const endTime = startTime + 1000;
            const result = adapter.testCreateResultSync('stdout', 'stderr', 0, undefined, 'command', startTime, endTime, { host: 'remote-host', container: 'my-container' });
