...
describe('Pipe Implementation', () => {
    ...
    describe('executePipe', () => {
        test('should pipe to string command', async () => {
            const sourceResult = new ExecutionResultImpl('hello world\n', '', 0, undefined, 'echo "hello world"', 100, new Date(), new Date(), 'local');
        ...
        test('should pipe to command object', async () => {
            const sourceResult = new ExecutionResultImpl('test data', '', 0, undefined, 'echo "test data"', 100, new Date(), new Date(), 'local');
        ...
        test('should pipe to template literal', async () => {
            const sourceResult = new ExecutionResultImpl('hello world', '', 0, undefined, 'echo "hello world"', 100, new Date(), new Date(), 'local');
            const template = Object.assign(['tr a-z A-Z'], { raw: ['tr a-z A-Z'] });
        ...
        test('should pipe to line processor function', async () => {
            const sourceResult = new ExecutionResultImpl('line1\nline2\nline3', '', 0, undefined, 'echo "lines"', 100, new Date(), new Date(), 'local');
        ...
        test('should handle custom line separator', async () => {
            const sourceResult = new ExecutionResultImpl('item1;item2;item3', '', 0, undefined, 'echo "items"', 100, new Date(), new Date(), 'local');
            const processedItems = [];
            const itemProcessor = (item) => {
                processedItems.push(item);
            };
            const result = await executePipe(Promise.resolve(sourceResult), itemProcessor, engine, { lineByLine: true, lineSeparator: ';' });
            expect(processedItems).toEqual(['item1', 'item2', 'item3']);
        });
        test('should throw on error when throwOnError is true', async () => {
            const sourceResult = new ExecutionResultImpl('', 'error', 1, undefined, 'false', 100, new Date(), new Date(), 'local');
            await expect(executePipe(Promise.resolve(sourceResult), 'cat', engine, { throwOnError: true })).rejects.toThrow('Previous command failed with exit code 1');
        });
        test('should not throw on error when throwOnError is false', async () => {
            const sourceResult = new ExecutionResultImpl('', 'error', 1, undefined, 'false', 100, new Date(), new Date(), 'local');
            const result = await executePipe(Promise.resolve(sourceResult), 'echo "recovered"', engine, { throwOnError: false });
    ...
    describe('pipeUtils', () => {
        test('should create uppercase transform', async () => {
            ...
            const sourceResult = new ExecutionResultImpl('hello world', '', 0, undefined, 'echo', 100, new Date(), new Date(), 'local');
        ...
        test('should create grep transform', async () => {
            ...
            const sourceResult = new ExecutionResultImpl('hello world\ngoodbye moon\nworld peace', '', 0, undefined, 'echo', 100, new Date(), new Date(), 'local');
            const result = await executePipe(Promise.resolve(sourceResult), transform, engine);
            expect(result.stdout.trim()).toBe('hello world\nworld peace');
        });
        test('should create grep transform with regex', async () => {
            const transform = pipeUtils.grep(/^world/);
            const sourceResult = new ExecutionResultImpl('hello world\nworld peace\ngoodbye world', '', 0, undefined, 'echo', 100, new Date(), new Date(), 'local');
            const result = await executePipe(Promise.resolve(sourceResult), transform, engine);
        ...
        test('should create replace transform with regex', async () => {
            ...
            const sourceResult = new ExecutionResultImpl('hello world and weird things', '', 0, undefined, 'echo', 100, new Date(), new Date(), 'local');
            const result = await executePipe(Promise.resolve(sourceResult), transform, engine);
        ...
        test('should create tee transform', async () => {
            ...
            const sourceResult = new ExecutionResultImpl('test data', '', 0, undefined, 'echo', 100, new Date(), new Date(), 'local');
            const result = await executePipe(Promise.resolve(sourceResult), transform, engine);
            expect(result.stdout).toBe('test data');
    ...
    describe('ProcessPromise.pipe integration', () => {
        ...
        test('should support complex pipe chains', async () => {
            const sourceResult = new ExecutionResultImpl('apple\nbanana\napple\ncherry', '', 0, undefined, 'printf "apple\\nbanana\\napple\\ncherry"', 100, new Date(), new Date(), 'local');
            ...
            const lines = [];
            await executePipe(Promise.resolve(grepResult), (line) => {
                lines.push(line);
            }, engine, { lineByLine: true });
            expect(lines).toEqual(['apple', 'apple']);
        });
    });
});
//# sourceMappingURL=pipe-implementation.test.js.map