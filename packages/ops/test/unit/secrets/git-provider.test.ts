...
describe('GitSecretProvider commit safety (defect #9)', () => {
  let repoDir: string;
  ...
  beforeEach(async () => {
    repoDir = await fs.mkdtemp(path.join(os.tmpdir(), 'xec-git-secrets-test-'));
  ...
  async function createProviderWithChange(secretsSubdir: string): Promise<GitSecretProvider> {
    const provider = new GitSecretProvider({
      repoPath: repoDir,
      secretsPath: secretsSubdir,
      autoCommit: false,
    });

    const secretsDir = path.join(repoDir, secretsSubdir);
    await fs.mkdir(secretsDir, { recursive: true });
    await fs.writeFile(path.join(secretsDir, 'data.enc'), 'ciphertext', 'utf-8');

    return provider;
  }

  it('commit messages with shell metacharacters are inert and preserved verbatim', async () => {
    const provider = await createProviderWithChange('.xec/secrets');
    const evil = 'secrets: update x"; touch pwned-by-message; echo "done';

    await (provider as unknown as { commitChanges(message: string): Promise<void> }).commitChanges(evil);

    expect(existsSync(path.join(repoDir, 'pwned-by-message'))).toBe(false);
    const lastMessage = git('log', '-1', '--format=%B').trim();
    expect(lastMessage).toBe(evil);
  });

  it('command substitution in commit messages is not executed', async () => {
    const provider = await createProviderWithChange('.xec/secrets');
    const evil = 'update $(touch pwned-by-subst) `touch pwned-by-backtick`';

    await (provider as unknown as { commitChanges(message: string): Promise<void> }).commitChanges(evil);

    expect(existsSync(path.join(repoDir, 'pwned-by-subst'))).toBe(false);
    expect(existsSync(path.join(repoDir, 'pwned-by-backtick'))).toBe(false);
    const lastMessage = git('log', '-1', '--format=%B').trim();
    expect(lastMessage).toBe(evil);
  });

  it('secrets paths containing spaces and metacharacters are staged safely', async () => {
    const provider = await createProviderWithChange('secret dir; touch pwned-by-path');

    await (provider as unknown as { commitChanges(message: string): Promise<void> }).commitChanges('secrets: path test');

    expect(existsSync(path.join(repoDir, 'pwned-by-path'))).toBe(false);
    const lastMessage = git('log', '-1', '--format=%B').trim();
    expect(lastMessage).toBe('secrets: path test');

    const committed = git('ls-tree', '-r', '--name-only', 'HEAD');
    expect(committed).toContain('secret dir; touch pwned-by-path/data.enc');
  });
});
...
describe('GitSecretProvider', () => {
  ...
  // A fresh repository per test: secrets accumulate in the repo, so a shared
  // repository would leak state between tests (list/export assert exact sets).
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), 'xec-test-git-secrets-'));
  ...
  describe('Phase 2: Team features', () => {
    describe('team member management', () => {
      ...
      it('should add team member', async () => {
        ...
        // initialize() auto-registers the current git user as an admin member,
        // so the list contains that user plus the member we just added.
        const members = await teamProvider.listTeamMembers();
        const alice = members.find(m => m.userId === 'alice@example.com');
        expect(alice).toBeDefined();
        expect(alice?.permissions).toBe('read-write');
      });
    ...
    describe('error handling', () => {
      ...
      it('should handle missing master key', async () => {
        await provider.initialize();
        await provider.set('existing-key', 'existing-value');

        // Remove master key — a new provider regenerates one, which makes the
        ...
        await expect(newProvider.get('existing-key')).rejects.toThrow(SecretError);
      });
    });
