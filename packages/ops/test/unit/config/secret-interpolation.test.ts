...
describe('Secret Interpolation Integration', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(os.tmpdir(), `xec-test-interpolation-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    ...
    // Create configuration file with secrets config. Secret references are
    // resolved strictly at load time, so the base config must not reference
  ...
  describe('basic secret interpolation', () => {
    ...
    it('should throw for missing secrets instead of silently dropping them', async () => {
      const template = 'Missing: ${secret:non-existent}';

      await expect(interpolator.interpolateAsync(template, {}))
        .rejects.toThrow(/secret 'non-existent' not found/);
    });

    it("should keep the literal for missing secrets with onUndefined: 'keep'", async () => {
      const template = 'Missing: ${secret:non-existent}';
      const result = await interpolator.interpolateAsync(template, {}, { onUndefined: 'keep' });

      expect(result).toBe('Missing: ${secret:non-existent}');
    });
  ...
  describe('configuration integration', () => {
    it('should resolve secrets in configuration', async () => {
      // Set secrets BEFORE loading — load-time interpolation is strict and
      // fails for secrets that do not exist.
      ...
      const targetResolver = freshManager.getTargetResolver();
    ...
    it('should fail loading a configuration referencing missing secrets', async () => {
      const config = {
        version: '2.0',
        secrets: {
          provider: 'local',
        ...
        targets: {
          hosts: {
            prod: { host: '${secret:never-created-host}' }
          }
        }
      };
      await fs.writeFile(configFile, JSON.stringify(config, null, 2));
      ...
      await expect(freshManager.load()).rejects.toThrow(/never-created-host/);
    });
  ...
  describe('sync vs async interpolation', () => {
    it('should throw in sync mode when the secret is not cached', async () => {
      await secretManager.set('sync-secret', 'sync-value');

      // Sync interpolation cannot reach the provider — it throws with a hint
      // to use interpolateAsync instead of injecting a placeholder.
      expect(() => interpolator.interpolate('${secret:sync-secret}', {}))
        .toThrow(/interpolateAsync/);

      // Async interpolation works and populates the cache...
      const asyncResult = await interpolator.interpolateAsync('${secret:sync-secret}', {});
      expect(asyncResult).toBe('sync-value');

      // ...after which sync interpolation succeeds from the cache
  ...
  describe('error handling', () => {
    it('should surface secret provider errors instead of silently returning nothing', async () => {
      ...
      await expect(errorInterpolator.interpolateAsync('${secret:fail}', {}))
        .rejects.toThrow(/Failed to retrieve secret 'fail'/);
    });

    it("should keep the literal on provider errors with onUndefined: 'keep'", async () => {
      const failingManager = new SecretManager();
      (failingManager as any).provider = {
        get: async () => { throw new Error('Provider error'); },
        initialize: async () => {}
      ...
      const result = await errorInterpolator.interpolateAsync('${secret:fail}', {}, { onUndefined: 'keep' });
      expect(result).toBe('${secret:fail}');
    });
