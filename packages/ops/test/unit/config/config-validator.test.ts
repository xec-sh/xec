...
describe('ConfigValidator secrets provider list (defect #8)', () => {
  ...
  const validateProvider = async (provider: string) => {
    ...
    return errors.filter(e => e.path === 'secrets.provider');
  };

  it('accepts every provider the SecretManager actually implements', async () => {
    expect(await validateProvider('local')).toHaveLength(0);
    expect(await validateProvider('env')).toHaveLength(0);
    expect(await validateProvider('git')).toHaveLength(0);
  });

  it('rejects declared-but-unimplemented providers with an actionable message', async () => {
    for (const provider of ['vault', '1password', 'aws-secrets', 'dotenv']) {
      const errors = await validateProvider(provider);
      expect(errors).toHaveLength(1);
      expect(errors[0]?.message).toMatch(/not yet implemented/);
      expect(errors[0]?.message).toMatch(/local, env, git/);
    }
  });

  it('rejects unknown providers', async () => {
    const errors = await validateProvider('mystery-vault');
    expect(errors).toHaveLength(1);
    expect(errors[0]?.message).toMatch(/Unknown secrets provider/);
    expect(errors[0]?.message).toMatch(/local, env, git/);
  });
});
...
describe('ConfigValidator', () => {
  ...
  describe('other validations', () => {
    // Provider name validation (accepted/unimplemented/unknown) is covered by
    // the 'secrets provider list (defect #8)' describe above.
