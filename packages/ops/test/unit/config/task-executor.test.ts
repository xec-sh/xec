...
function createExecutor(tasks: Record<string, TaskDefinition> = {}, options: { dryRun?: boolean } = {}): TaskExecutor {
  return new TaskExecutor({
    interpolator: new VariableInterpolator(),
    targetResolver: new TargetResolver(baseConfig),
    dryRun: options.dryRun,
    taskRegistry: (name) => tasks[name],
  });
}
...
describe('TaskExecutor', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'xec-executor-test-'));
  });
  ...
  describe('when: conditions (defect #2)', () => {
    it('runs the step when the condition is true', async () => {
      const executor = createExecutor();
      const marker = path.join(tmpDir, 'ran-true');

      const result = await executor.execute('t', {
        steps: [{ name: 's1', command: `touch ${marker}`, when: "${vars.deploy} == 'yes'" }],
      }, { vars: { deploy: 'yes' }, quiet: true });
    ...
    it('skips the step (marked skipped, command not executed) when the condition is false', async () => {
      const executor = createExecutor();
      const marker = path.join(tmpDir, 'ran-false');

      const result = await executor.execute('t', {
        steps: [{ name: 's1', command: `touch ${marker}`, when: "${vars.deploy} == 'yes'" }],
      }, { vars: { deploy: 'no' }, quiet: true });
    ...
    it('FAILS the step (never skip-as-success) when the condition references an undefined variable', async () => {
      const executor = createExecutor();
      const marker = path.join(tmpDir, 'ran-broken');

      const result = await executor.execute('t', {
        steps: [{ name: 's1', command: `touch ${marker}`, when: "${vars.never_defined} == 'yes'" }],
      }, { vars: {}, quiet: true });

      expect(result.success).toBe(false);
      expect(result.steps?.[0]?.success).toBe(false);
      expect(result.steps?.[0]?.error?.message).toMatch(/never_defined/);
      expect(existsSync(marker)).toBe(false);
    });

    it('FAILS the step when the condition expression is malformed', async () => {
      const executor = createExecutor();

      const result = await executor.execute('t', {
        steps: [{ name: 's1', command: 'echo hi', when: '${vars.x} ==' }],
      }, { vars: { x: 1 }, quiet: true });

      expect(result.success).toBe(false);
      expect(result.steps?.[0]?.success).toBe(false);
      expect(result.steps?.[0]?.error?.message).toMatch(/Cannot evaluate condition/);
    });
    ...
    it('supports complex expressions', async () => {
      ...
      const result = await executor.execute('t', {
        steps: [{
          name: 's1',
          command: 'echo ok',
          when: "(${vars.env} == 'prod' || ${vars.force} == true) && ${vars.count} > 2",
        }],
      }, { vars: { env: 'dev', force: true, count: 3 }, quiet: true });

      expect(result.success).toBe(true);
      expect(result.steps?.[0]?.skipped).toBeUndefined();
      expect(result.steps?.[0]?.output).toContain('ok');
    });
  });

  describe('nested tasks (defect #3)', () => {
    it('actually executes the nested task and captures its output', async () => {
      const marker = path.join(tmpDir, 'child-ran');
      const tasks: Record<string, TaskDefinition> = {
        child: { command: `echo child-output && touch ${marker}` },
      };
      const executor = createExecutor(tasks);

      const result = await executor.execute('parent', {
        steps: [{ name: 'call-child', task: 'child' }],
      }, { quiet: true });

      expect(result.success).toBe(true);
      expect(existsSync(marker)).toBe(true);
      expect(result.steps?.[0]?.output).toContain('child-output');
      expect(result.steps?.[0]?.output).not.toContain('Would execute');
    });

    it('fails loudly when no task registry is available', async () => {
      ...
      const result = await executor.execute('parent', {
        steps: [{ name: 's1', task: 'child' }],
      }, { quiet: true });

      expect(result.success).toBe(false);
      expect(result.steps?.[0]?.success).toBe(false);
      expect(result.steps?.[0]?.error?.message).toMatch(/task registry/);
    });

    it('fails when the nested task does not exist', async () => {
      const executor = createExecutor({});
      ...
      expect(result.success).toBe(false);
      expect(result.steps?.[0]?.error?.message).toMatch(/'ghost' not found/);
    });

    it('propagates nested task failure instead of faking success', async () => {
      const tasks: Record<string, TaskDefinition> = {
        failing: { command: 'exit 7' },
      };
      const executor = createExecutor(tasks);

      const result = await executor.execute('parent', {
        steps: [{ name: 's1', task: 'failing' }],
      }, { quiet: true });

      expect(result.success).toBe(false);
      expect(result.steps?.[0]?.success).toBe(false);
      expect(result.steps?.[0]?.error?.message).toMatch(/Nested task 'failing' failed/);
    });

    it('detects circular task invocation', async () => {
      const tasks: Record<string, TaskDefinition> = {
        a: { steps: [{ name: 'to-b', task: 'b' }] },
        b: { steps: [{ name: 'to-a', task: 'a' }] },
      };
      const executor = createExecutor(tasks);

      const result = await executor.execute('a', tasks['a']!, { quiet: true });

      expect(result.success).toBe(false);
      const message = result.steps?.[0]?.error?.message ?? '';
      expect(message).toMatch(/failed|Circular/);
    });

    it('rejects self-recursion directly', async () => {
      const tasks: Record<string, TaskDefinition> = {
        self: { steps: [{ name: 'again', task: 'self' }] },
      };
      const executor = createExecutor(tasks);

      const result = await executor.execute('self', tasks['self']!, { quiet: true });

      expect(result.success).toBe(false);
      expect(result.steps?.[0]?.error?.message).toMatch(/Circular task invocation/);
    });

    it('applies nested task parameter defaults and passes parent params through', async () => {
      const tasks: Record<string, TaskDefinition> = {
        child: {
          params: [
      ...
      const result = await executor.execute('parent', {
        steps: [{ name: 's1', task: 'child' }],
      }, { params: { name: 'world' }, quiet: true });

      expect(result.success).toBe(true);
      expect(result.steps?.[0]?.output).toContain('hello-world');
    });

    it('fails when a required nested parameter is missing', async () => {
      const tasks: Record<string, TaskDefinition> = {
        child: {
      ...
      expect(result.success).toBe(false);
      expect(result.steps?.[0]?.error?.message).toMatch(/missing required parameter.*name/);
    });

    it('honours dry-run for nested tasks', async () => {
      const marker = path.join(tmpDir, 'dry-child');
      const tasks: Record<string, TaskDefinition> = {
        child: { command: `touch ${marker}` },
      };
      const executor = createExecutor(tasks, { dryRun: true });
  ...
  describe('environment forwarding (defect #6)', () => {
    it('forwards only explicitly configured env, never the whole process.env', async () => {
      process.env['XEC_TEST_LEAKY_TOKEN'] = 'should-not-be-forwarded';
      try {
        const executor = createExecutor();
        const context = { params: {}, vars: { region: 'eu-1' }, env: { XEC_TEST_LEAKY_TOKEN: 'should-not-be-forwarded' } };

        const execEnv = await (executor as any).buildExecutionEnv(
          context,
          { TASK_VAR: 'from-task', REGION: '${vars.region}' },
          { STEP_VAR: 'from-step' },
          { CALLER_VAR: 'from-caller', TASK_VAR: 'overridden' }
        );

        expect(execEnv).toEqual({
          TASK_VAR: 'overridden',
          REGION: 'eu-1',
          STEP_VAR: 'from-step',
          CALLER_VAR: 'from-caller',
        });
        expect(execEnv['XEC_TEST_LEAKY_TOKEN']).toBeUndefined();
        expect(Object.keys(execEnv)).not.toContain('PATH');
      } finally {
        delete process.env['XEC_TEST_LEAKY_TOKEN'];
      }
    });

    it('applies task-level env to executed commands', async () => {
      const executor = createExecutor();

      const result = await executor.execute('t', {
        command: 'sh -c "echo marker=$XEC_TASK_ENV_TEST"',
        env: { XEC_TASK_ENV_TEST: 'task-env-value' },
      }, { quiet: true });

      expect(result.success).toBe(true);
      expect(result.output).toContain('marker=task-env-value');
    });

    it('applies step-level env to step commands', async () => {
      const executor = createExecutor();

      const result = await executor.execute('t', {
        steps: [{
          name: 's1',
          command: 'sh -c "echo marker=$XEC_STEP_ENV_TEST"',
          env: { XEC_STEP_ENV_TEST: 'step-env-value' },
        }],
      }, { quiet: true });

      expect(result.success).toBe(true);
      expect(result.steps?.[0]?.output).toContain('marker=step-env-value');
    });

    it('interpolation context still sees the local environment for ${env.X}', async () => {
      process.env['XEC_TEST_INTERP_ENV'] = 'visible-locally';
      try {
        const executor = createExecutor();

        const result = await executor.execute('t', {
          command: 'echo value=${env.XEC_TEST_INTERP_ENV}',
        }, { quiet: true });

        expect(result.success).toBe(true);
        expect(result.output).toContain('value=visible-locally');
      } finally {
        delete process.env['XEC_TEST_INTERP_ENV'];
      }
    });
  });

  describe('command interpolation is strict at runtime', () => {
    it('fails the task when a command references an undefined variable', async () => {
      const executor = createExecutor();

      const result = await executor.execute('t', {
        command: 'deploy --host ${vars.missing_host}',
      }, { vars: {}, quiet: true });

      expect(result.success).toBe(false);
      expect(result.error?.message).toMatch(/missing_host/);
    });
  });
});

describe('TaskExecutor execution behaviours', () => {
