...
function configurationInvalid(field: string, message: string): Error {
  return new Error(`Configuration error [${field}]: ${message}`);
}
...
export function parseTimeout(timeout: string | number): number {
  ...
  if (!match) {
    throw configurationInvalid('timeout', `Invalid timeout format: ${timeout}`);
  }
...
export function parseInterval(interval: string): {
  ...
  throw configurationInvalid('interval', `Invalid interval format: ${interval}`);
}
