...
function previousWordStart(text: string, caret: number): number {
  ...
  while (i > 0) {
    const left = text[i - 1]
    // When at the end of text, treat it as continuing the same character type
    const right = i < text.length ? text[i] : (i > 0 ? text[i - 1] : "")

    // First iteration: skip single punctuation if preceded by non-punct
    if (isFirstIteration && i < text.length && isPunctuation(right) && !isPunctuation(left) && left !== "\n") {
      isFirstIteration = false
      i--
      continue
    }
    ...
    const leftKind = kindOf(left)
    const rightKind = i < text.length ? kindOf(right) : leftKind
