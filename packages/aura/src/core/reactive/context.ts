...
export class ComputationImpl implements Computation {
  ...
  invalidate(): void {
    ...
    if (batchDepth > 0) {
      ...
      } else if (this.type === ComputationType.RESOURCE || inSyncContext) {
        // Run resources and effects in sync context immediately
        this.run();
      } else {
        // For other effects, schedule asynchronously to prevent infinite loops
  ...
  batch(fn: () => void): void {
    if (batchDepth === 0) {
      ...
      try {
        ...
        // Ensure batch is properly closed even on error
        throw error;
      } finally {
        batchDepth--;
        inSyncContext = prevSyncContext;
  ...
  private flushBatch(): void {
    ...
    if (computeds.length > 0) {
      ...
      for (const comp of sorted) {
        // For computeds, we need to trigger actual recomputation
        // First run the computation to ensure it's marked properly
        (comp as ComputationImpl).run();
        // Then access the computed to trigger recomputation if stale
        const computedImpl = (comp as any).__computed;
        if (computedImpl && typeof computedImpl.get === 'function') {
          // Use untrack to avoid creating new dependencies
