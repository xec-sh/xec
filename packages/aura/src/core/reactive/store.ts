...
export interface DeepStoreOptions {
...
export class Store<T extends object> {
  ...
  private proxyRefs = new Map<string, WeakRef<any>>();
  ...
  private options: DeepStoreOptions;
  private version = 0;
  private globalSubscribers: Array<(state: T) => void> = [];

  constructor(initial: T, options: DeepStoreOptions = {}) {
    this.options = {
      equals: Object.is,
      ...options
    };
  ...
  private getOrCreateProxy(obj: object, path: string[]): any {
    ...
    const ref = this.proxyRefs.get(pathStr);
    ...
    this.proxyRefs.set(pathStr, new WeakRef(proxy));
    return proxy;
  }
  ...
  private createProxy<O extends object>(obj: O, path: string[], storeMethods?: any): O {
    ...
    const proxy = new Proxy(obj, {
      get: (target, prop, receiver) => {
        // Special properties
        if (prop === Symbol.toStringTag) return 'DeepStore';
  ...
  private createArrayProxy<T>(arr: T[], path: string[], _storeMethods?: any): T[] {
