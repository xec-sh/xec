...
And everything is blue`
  ...
  const houseDisplay = new TextComponent(renderer.root.ctx, {
    id: "house-text",
    content: houseText,
    width: 30,
    height: 6,
...
${bgYellow(fg("black")(" NOTICE "))} System update available`
  ...
  const statusDisplay = new TextComponent(renderer.root.ctx, {
    id: "status-text",
    content: statusText,
    width: 50,
    height: 6,
  ...
  dashboardBox = new BoxComponent(renderer.root.ctx, {
    id: "dashboard-box",
    width: 72,
    height: 21,
    position: "absolute",
...
${bold(fg("#F1C40F")("Controls:"))} ${fg("#BDC3C7")("↑/↓ = Speed, ESC = Exit")}`
  ...
  complexDisplay = new TextComponent(renderer.root.ctx, {
    id: "complex-template",
    content: initialText,
    left: 1,
    top: 1,
...
${underline("Dynamic:")} ${bold(fg("#FF6B6B")(Math.sin(counter * 0.1) > 0 ? "UP" : "DOWN"))}`
      ...
      if (dynamicDisplay) {
        ...
        const newDynamicDisplay = new TextComponent(renderer.root.ctx, {
          id: "dynamic-text",
          content: dynamicText,
          width: 40,
          height: 4,
...
• Complex templates with ${red("many variables")}`
  ...
  const instructionsDisplay = new TextComponent(renderer.root.ctx, {
    id: "instructions",
...
Calculated: ${fg("#00FFFF")(Math.floor(Math.random() * 100))}`
  ...
  const typesDisplay = new TextComponent(renderer.root.ctx, {
    id: "types-text",
    content: typesText,
    width: 30,
    height: 6,
