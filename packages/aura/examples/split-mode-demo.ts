...
let instructionsText: TextComponent | null = null
...
class SplitModeAnimations {
  ...
  private movingOrbs: BoxComponent[] = []
  private statusCounters: TextComponent[] = []
  private pulsingElements: BoxComponent[] = []
  ...
  constructor(renderer: CliRenderer) {
    ...
    this.container = new GroupComponent("animation-container", {
      zIndex: 5,
      visible: true,
    })
  ...
  private setupUI(): void {
    const statusPanel = new BoxComponent("status-panel", {
      position: "absolute",
      left: 2,
      top: 5,
    ...
    systems.forEach((system, index) => {
      const label = new TextComponent(`${system.name.toLowerCase()}-label`, {
        content: `${system.name}:`,
        position: "absolute",
        left: 4,
        top: system.y,
      ...
      const bgBar = new BoxComponent(`${system.name.toLowerCase()}-bg`, {
        position: "absolute",
        left: 9,
        top: system.y,
      ...
      const progressBar = new BoxComponent(`${system.name.toLowerCase()}-progress`, {
        position: "absolute",
        left: 9,
        top: system.y,
        width: 1,
    ...
    const statsPanel = new BoxComponent("stats-panel", {
      position: "absolute",
      left: 2,
      top: 14,
      width: this.renderer.width - 6,
    ...
    counterLabels.forEach((label, index) => {
      const counter = new TextComponent(`counter-${index}`, {
        content: `${label}: 0`,
        position: "absolute",
        left: 4 + index * 15,
        top: 15,
    ...
    orbColors.forEach((color, index) => {
      const orb = new BoxComponent(`orb-${index}`, {
        position: "absolute",
        left: 2,
        top: 2,
        width: 3,
    ...
    pulseColors.forEach((color, index) => {
      const pulse = new BoxComponent(`pulse-${index}`, {
        position: "absolute",
...
export function run(rendererInstance: CliRenderer): void {
  ...
  text = new TextComponent("demo-text", {
    position: "absolute",
    left: 2,
    top: 0,
  ...
  instructionsText = new TextComponent("split-mode-instructions", {
    position: "absolute",
    left: 2,
    top: 19,
    width: rendererInstance.width - 4,
