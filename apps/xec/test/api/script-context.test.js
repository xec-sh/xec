...
describe('Script Context', () => {
    ...
    beforeEach(async () => {
        ...
        tasks.targetResolver = undefined;
    });
    afterEach(async () => {
    ...
    describe('inject() and cleanup()', () => {
        it('should inject and cleanup global context', async () => {
            const scriptPath = path.join(tempDir, 'test.js');
            const context = await ScriptContext.create(scriptPath);
            expect(global.$target).toBeUndefined();
            expect(global.config).toBeUndefined();
            ScriptContext.inject(context);
            expect(global.$target).toBe($);
            expect(global.config).toBeDefined();
            expect(global.chalk).toBeDefined();
            ScriptContext.cleanup(context);
            expect(global.$target).toBeUndefined();
            expect(global.config).toBeUndefined();
            expect(global.chalk).toBeUndefined();
        });
    });
...
//# sourceMappingURL=script-context.test.js.map