...
import { enhanceError, EnhancedCommandError, EnhancedTimeoutError, EnhancedExecutionError, EnhancedConnectionError } from '../../src/utils/enhanced-error.js';
describe('Enhanced Error System', () => {
    describe('EnhancedExecutionError', () => {
        it('should create error with basic properties', () => {
            const error = new EnhancedExecutionError('Test error message', 'TEST_ERROR', { command: 'test-command', adapter: 'local' }, [{ message: 'Try this instead' }]);
        ...
        it('should format error for display', () => {
            const error = new EnhancedExecutionError('Command failed', 'CMD_FAILED', {
                command: 'npm test',
                adapter: 'docker',
                container: 'myapp',
                duration: 5000
    ...
    describe('EnhancedCommandError', () => {
        it('should generate suggestions for exit code 127', () => {
            const error = new EnhancedCommandError('unknown-cmd', 127, undefined, '', 'unknown-cmd: command not found', 1000, 'local');
        ...
        it('should generate suggestions for permission denied', () => {
            const error = new EnhancedCommandError('cat /etc/shadow', 1, undefined, '', 'cat: /etc/shadow: Permission denied', 100, 'local');
            const suggestions = error.suggestions;
        ...
        it('should generate adapter-specific suggestions for Docker', () => {
            const error = new EnhancedCommandError('python script.py', 127, undefined, '', 'python: not found', 200, 'docker');
            const suggestions = error.suggestions;
        ...
        it('should generate adapter-specific suggestions for Kubernetes', () => {
            const error = new EnhancedCommandError('kubectl version', 127, undefined, '', 'kubectl: not found', 300, 'kubernetes');
            const suggestions = error.suggestions;
        ...
        it('should generate suggestions for exit code 128 (invalid argument)', () => {
            const error = new EnhancedCommandError('exit 256', 128, undefined, '', 'exit: 256: numeric argument required', 50, 'local');
            const suggestions = error.suggestions;
    ...
    describe('EnhancedConnectionError', () => {
        it('should generate suggestions for DNS errors', () => {
            const error = new EnhancedConnectionError('unknown.host.com', new Error('getaddrinfo ENOTFOUND unknown.host.com'), 22, 'ssh');
            const suggestions = error.suggestions;
    ...
    describe('Error suggestions quality', () => {
        it('should provide helpful install commands', () => {
            ...
            const installSuggestion = error.suggestions.find(s => s.message.includes('install') && s.command && !s.command.includes('which'));
        ...
        it('should provide helpful file not found suggestions', () => {
            const error = new EnhancedCommandError('cat config.json', 1, undefined, '', 'cat: config.json: No such file or directory', 50);
            const suggestions = error.suggestions;
        ...
        it('should handle signal-based termination', () => {
            const error = new EnhancedCommandError('sleep 100', null, 'SIGTERM', '', '', 5000);
            expect(error.message).toContain('exit code null');
            expect(error.context.duration).toBe(5000);
        });
    });
});
//# sourceMappingURL=enhanced-error.test.js.map