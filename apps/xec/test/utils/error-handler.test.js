...
import { XecError, TaskError, ModuleError, RecipeError, handleError, NetworkError, TimeoutError, errorMessages, FileSystemError, withErrorHandling, ConfigurationError, createContextError } from '../../src/utils/error-handler.js';
...
describe('error-handler', () => {
    ...
    describe('handleError', () => {
        ...
        it('should show debug info when XEC_DEBUG is set', () => {
            ...
            const hasDebugInfo = errorCalls.some(call => call[0] && typeof call[0] === 'string' && call[0].includes('=== DEBUG ERROR INFO ==='));
            const hasDebugCode = errorCalls.some(call => call.some(arg => arg && typeof arg === 'string' && arg.includes('DEBUG_CODE')));
        ...
        it('should display verbose error info', () => {
            ...
            expect(() => handleError(error, options)).toThrow('Process exited with code: 1');
            expect(mockConsoleError.mock.calls.some(call => call[0]?.includes && call[0].includes('Run with --verbose'))).toBe(false);
        });
    });
    describe('Exit Codes', () => {
