...
describe('Phase 1 and Phase 2 Feature Verification', () => {
    ...
    describe('Phase 2: Target System', () => {
        it('should apply target defaults correctly', async () => {
            ...
            expect(server1.config.port).toBe(2222);
            expect(server1.config.timeout).toBe(5000);
            expect(server1.config.shell).toBe('/bin/bash');
            expect(server1.config.keepAlive).toBe(true);
            const server2 = await resolver.resolve('hosts.server2');
            expect(server2.config.port).toBe(22);
            expect(server2.config.timeout).toBe(10000);
            expect(server2.config.keepAlive).toBe(true);
            const app = await resolver.resolve('containers.app');
            expect(app.config.tty).toBe(true);
            expect(app.config.workdir).toBe('/app');
            const db = await resolver.resolve('containers.db');
            expect(db.config.tty).toBe(false);
            expect(db.config.workdir).toBe('/app');
        });
    ...
    describe('Phase 2: Profile System', () => {
        it('should handle profile inheritance with extends', async () => {
            ...
            expect(loaded.vars?.profileVar).toBe('from dev');
            expect(loaded.vars?.baseOnly).toBe('base value');
            ...
            loaded = await manager.load();
            expect(loaded.vars?.overrideVar).toBe('staging');
            expect(loaded.vars?.profileVar).toBe('from dev');
            expect(loaded.vars?.baseOnly).toBe('base value');
            expect(loaded.vars?.devOnly).toBe('dev value');
        ...
        it('should handle $unset marker in profiles', async () => {
            ...
            expect(loaded.vars?.logLevel).toBe('error');
        });
    });
    describe('Phase 2: Configuration Import', () => {
        it('should merge global and project configurations', async () => {
            ...
            expect(loaded.vars?.sharedVar).toBe('from project');
...
//# sourceMappingURL=phase-verification.test.js.map