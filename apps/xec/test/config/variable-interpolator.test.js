...
describe('VariableInterpolator', () => {
    ...
    describe('interpolate()', () => {
        it('should handle non-string values', () => {
            expect(interpolator.interpolate(123, context)).toBe(123);
            expect(interpolator.interpolate(true, context)).toBe(true);
            expect(interpolator.interpolate(null, context)).toBe(null);
            expect(interpolator.interpolate(undefined, context)).toBe(undefined);
        });
        ...
        it('should handle multiple variables in one string', () => {
            const result = interpolator.interpolate('App: ${vars.appName} v${vars.version} on ${params.environment}', context);
        ...
        it('should handle non-string values in async interpolation', async () => {
            expect(await interpolator.interpolateAsync(123, context)).toBe(123);
            expect(await interpolator.interpolateAsync(true, context)).toBe(true);
            expect(await interpolator.interpolateAsync(null, context)).toBe(null);
        });
        ...
        it('should handle complex commands with file operations', async () => {
            ...
            try {
                const writeResult = await interpolator.interpolateAsync(`\${cmd:echo "test content from interpolator" > ${tempFile} && echo "success"}`, context);
    ...
    describe('parseVariables()', () => {
        ...
        it('should parse all variables from string', () => {
            const variables = interpolator.parseVariables('App ${vars.name} v${vars.version} env: ${env.NODE_ENV:development}');
    ...
    describe('resolveConfig()', () => {
        it('should resolve variables in entire configuration', async () => {
            ...
            expect(resolved.tasks?.deploy.command).toBe('kubectl apply -f myapp.yaml');
            expect(resolved.tasks?.deploy.description).toBe('Deploy myapp to cluster');
...
//# sourceMappingURL=variable-interpolator.test.js.map