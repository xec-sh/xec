...
import { TargetResolver, ConfigurationManager } from '../../src/config/index';
  ...
  urls:
    ...
    describe('Error handling and validation', () => {
        it('should handle validation errors gracefully', async () => {
            const invalidConfig = `version: "1.0"  # Old version
vars:
  # Reserved name
  env: production
...
tasks:
  ...
  invalid-ref:
    ...
    target: nonexistent.target`;
            ...
            expect(warnings.some(w => w.includes('Version 1.0 is not supported'))).toBe(true);
  ...
  containers:
    configured-app:
      image: node:18`;
            ...
            const loaded = await manager.load();
            const resolver = new TargetResolver(loaded);
            const isDockerContainerSpy = jest.spyOn(resolver, 'isDockerContainer')
