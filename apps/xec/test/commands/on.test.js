...
class TestableOnCommand extends OnCommand {
    constructor(options = {}) {
        ...
        this.taskCalls = [];
        this.replCalls = [];
        this.findTargetsResults = new Map();
        this.createdEngines = [];
        this.outputLines = [];
        this.errorLines = [];
    ...
    async initializeConfig(options) {
        ...
        if (this.options.mockExecute) {
            this.taskManager = {
                run: async (taskName, variables, context) => {
    ...
    async createTargetEngine(target) {
        if (this.options.mockExecute) {
            const self = this;
            const execFunction = async function (strings, ...values) {
                let command = '';
            ...
            execFunction.raw = async function (strings, ...values) {
                let command = '';
                for (let i = 0; i < strings.length; i++) {
                    command += strings[i];
                    if (i < values.length) {
            ...
            execFunction.env = function (vars) {
                execFunction._options.env = { ...execFunction._options.env, ...vars };
                return execFunction;
            };
            execFunction.cd = function (dir) {
                execFunction._options.cwd = dir;
                return execFunction;
            };
            execFunction.timeout = function (ms) {
    ...
    async resolveTargetsForTest(hostPattern) {
        let targets;
...
describe('On Command', () => {
    ...
    describe('Error Handling', () => {
        ...
        it('should handle command execution failures', async () => {
            ...
            await fs.writeFile(path.join(projectDir, '.xec', 'config.yaml'), yaml.dump(config));
            await expect(command.execute(['hosts.test-server', 'failing-command', { quiet: true }])).rejects.toThrow('Command execution failed');
        });
    ...
    describe('Edge Cases', () => {
        ...
        it('should handle REPL mode when multiple hosts specified', async () => {
            ...
            await fs.writeFile(path.join(projectDir, '.xec', 'config.yaml'), yaml.dump(config));
            await expect(command.execute(['hosts.server-*', { repl: true, quiet: true }])).rejects.toThrow('REPL mode is only supported for single hosts');
        });
    });
    ...
    describe('No Command Specified', () => {
        ...
        it('should throw error when no command, task, or REPL is specified', async () => {
            ...
            await fs.writeFile(path.join(projectDir, '.xec', 'config.yaml'), yaml.dump(config));
            await expect(command.execute(['hosts.test-host', { quiet: true }])).rejects.toThrow('No command, task, or REPL mode specified');
        });
    });
    ...
    describe('Output Formatting', () => {
        ...
        it('should handle verbose stderr output', async () => {
            ...
            command.createTargetEngine = async function (target) {
                const execFunction = {
                    async raw() {
                        return {
        ...
        it('should handle empty targets gracefully', async () => {
            ...
            await fs.writeFile(path.join(projectDir, '.xec', 'config.yaml'), yaml.dump(config));
            await expect(command.execute(['hosts.non-existent-*', { repl: true, quiet: true }])).rejects.toThrow('No hosts found matching pattern');
        });
    });
    describe('Dry Run Mode', () => {
        it('should log dry run info without executing', async () => {
            ...
            expect(command.outputLines.some(line => line.includes('[DRY RUN]') &&
                line.includes('Would execute') &&
                line.includes('rm -rf /important'))).toBe(true);
        });
