...
describe('inspect command', () => {
  ...
  describe('output formats', () => {
    ...
    it('should output YAML format', async () => {
      ...
      // Check YAML-like output - vars section
      // YAML format has key: on one line and value on next line
      expect(output).toMatch(/app_name:\s*\n\s*test-app/);
      expect(output).toMatch(/version:\s*\n\s*1\.0\.0/);
    });
  ...
  describe('filtering', () => {
    ...
    it('should show no items when filter matches nothing', async () => {
      await program.parseAsync(['node', 'xec', 'inspect', 'all', '--filter', 'nonexistent']);
  ...
  describe('task inspection', () => {
    ...
    it('should show private tasks only in verbose mode', async () => {
      // Without verbose
      await program.parseAsync(['node', 'xec', 'inspect', 'tasks']);
      let output = getConsoleOutput();
      ...
      // With verbose
      await program.parseAsync(['node', 'xec', 'inspect', 'tasks', '--verbose']);
      output = getConsoleOutput();
      // Private tasks might still be filtered by task manager
    });
  ...
  describe('script inspection', () => {
    ...
    it('should show script details with verbose flag', async () => {
      await program.parseAsync(['node', 'xec', 'inspect', 'scripts', '--verbose']);
      
      expect(consoleLogSpy).toHaveBeenCalled();
      const output = getConsoleOutput();
      
      // Should extract and show description in table
      expect(output).toMatch(/deploy\.js|Deploy the application/);
    });
  ...
  describe('system inspection', () => {
    ...
    it('should show OS information only', async () => {
      ...
      expect(output).toMatch(/Platform|Architecture|macOS|Linux|Windows|darwin|linux|win32/);
    });
  ...
  describe('verbose mode', () => {
    it('should show additional details in verbose mode', async () => {
      await program.parseAsync(['node', 'xec', 'inspect', 'all', '--verbose']);
      ...
      consoleLogSpy.mockClear();
      await program.parseAsync(['node', 'xec', 'inspect', 'all']);
      const nonVerboseOutput = getConsoleOutput();
      
      // Verbose should generally have more content
      expect(output.length).toBeGreaterThanOrEqual(nonVerboseOutput.length);
    }, 10000);

    it('should show metadata in verbose tree format', async () => {
      await program.parseAsync(['node', 'xec', 'inspect', 'all', '--format', 'tree', '--verbose']);
      
      expect(consoleLogSpy).toHaveBeenCalled();
      const output = getConsoleOutput();
      
      // Should show metadata entries
      expect(output).toMatch(/│\s+└─/); // nested tree items
    }, 10000);
  });
  ...
  describe('interactive mode', () => {
    it.skip('should enter interactive mode when requested - skipped due to mock timeout issue', async () => {
      // The @clack/prompts mock is already set up to return 'exit'
      await program.parseAsync(['node', 'xec', 'inspect', '--interactive']);
      
      expect(consoleLogSpy).toHaveBeenCalled();
      const output = getConsoleOutput();
      
      // Should show interactive prompt header
      expect(output).toMatch(/Xec Project Inspector/);
    }, 15000); // Increase timeout for interactive mode
  });
  ...
  describe('error handling', () => {
    ...
    it('should show stack trace in verbose mode', async () => {
      ...
      try {
        ...
        // Should have warned about the invalid config with more details
        expect(consoleWarnSpy).toHaveBeenCalled();
