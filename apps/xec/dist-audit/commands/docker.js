...
export class DockerCommand extends SubcommandBase {
    ...
    setupContainerCommands(docker) {
        ...
        container
            ...
            .action(async (image, options, cmd) => {
        ...
        container
            ...
            .option('--user <user>', 'User to run as')
            .option('--workdir <dir>', 'Working directory')
            .action(async (containerName, cmdParts, options, cmd) => {
            const parentOpts = cmd.parent?.parent?.opts() || {};
            const mergedOpts = { ...parentOpts, ...options };
        ...
        container
            ...
            .action(async (containers, options, cmd) => {
            const parentOpts = cmd.parent?.parent?.opts() || {};
            const mergedOpts = { ...parentOpts, ...options };
    ...
    setupImageCommands(docker) {
        ...
        image
            ...
            .action(async (imageName, options) => {
    ...
    async runContainer(image, options) {
        ...
        try {
            ...
            if (mergedOptions.name)
                container = container.name(mergedOptions.name);
            if (mergedOptions.workdir)
                container = container.workdir(mergedOptions.workdir);
            if (mergedOptions.user)
                container = container.user(mergedOptions.user);
            if (mergedOptions.network)
                container = container.network(mergedOptions.network);
            if (mergedOptions.rm)
                container = container.autoRemove();
            if (mergedOptions.privileged)
            ...
            if (mergedOptions.dryRun) {
                ...
                if (mergedOptions.name)
                    this.log(`[DRY RUN] Name: ${mergedOptions.name}`, 'info');
                return;
            }
    ...
    async execInContainer(containerName, cmdParts, options) {
        await this.initializeConfig(options);
        try {
            const dockerApi = $.docker();
            let container = dockerApi.container(containerName);
            if (options.user)
                container = container.user(options.user);
            if (options.workdir)
                container = container.workdir(options.workdir);
            if (this.isDryRun()) {
    ...
    async stopContainers(containers, options) {
        await this.initializeConfig(options);
        const s = spinner();
    ...
    async removeContainers(containers, options) {
        await this.initializeConfig(options);
        const s = spinner();
    ...
    async viewContainerLogs(containerName, options) {
        await this.initializeConfig(options);
        try {
    ...
    async buildImage(context, options) {
        ...
        try {
            ...
            if (options.file)
                builder = builder.dockerfile(options.file);
            if (options.noCache)
                builder = builder.noCache();
            if (options.pull)
                builder = builder.pull();
            if (options.platform)
                builder = builder.platform(options.platform);
            if (options.target)
                builder = builder.target(options.target);
            if (options.buildArg) {
            ...
            if (this.isDryRun()) {
                ...
                if (options.tag)
                    this.log(`[DRY RUN] Tag: ${options.tag}`, 'info');
                return;
            }
    ...
    async removeImages(images, options) {
        await this.initializeConfig(options);
        const s = spinner();
    ...
    async startRedisService(options) {
        this.options = { ...this.options, ...options };
        await this.initializeConfig(options);
        const s = spinner();
...
Connection: ${info['connectionString']}
    ...
    async startPostgresService(options) {
        this.options = { ...this.options, ...options };
        await this.initializeConfig(options);
        const s = spinner();
...
Connection: ${info['connectionString']}
    ...
    async startMysqlService(options) {
        this.options = { ...this.options, ...options };
        await this.initializeConfig(options);
        const s = spinner();
...
Connection: ${info['connectionString']}
    ...
    async startMongoDBService(options) {
        this.options = { ...this.options, ...options };
        await this.initializeConfig(options);
        const s = spinner();
...
Connection: ${info['connectionString']}
    ...
    async startRedisClusterService(options) {
        this.options = { ...this.options, ...options };
        await this.initializeConfig(options);
        const s = spinner();
...
Connect with:
  redis-cli -c -p ${options.basePort || '7001'}
    ...
    async startKafkaService(options) {
        this.options = { ...this.options, ...options };
        await this.initializeConfig(options);
        const s = spinner();
...
Connect with:
  kafka-topics --bootstrap-server localhost:${mergedOptions.port || '9092'} --list
    ...
    async startRabbitMQService(options) {
        this.options = { ...this.options, ...options };
        await this.initializeConfig(options);
        const s = spinner();
...
Password: ${mergedOptions.password || 'admin'}
    ...
    async startElasticsearchService(options) {
        this.options = { ...this.options, ...options };
        await this.initializeConfig(options);
        const s = spinner();
  ...
  ssh -p ${info['port']} ${info['username']}@localhost
    ...
    async composeUp(options) {
        ...
        try {
            ...
            if (options.project)
                compose = compose.withProject(options.project);
            if (options.profiles)
                compose = compose.withProfiles(...options.profiles);
            if (options.env) {
            ...
            if (this.isDryRun()) {
                ...
                if (options.project)
                    this.log(`[DRY RUN] Project: ${options.project}`, 'info');
                return;
            }
    ...
    async createNetwork(name, options) {
        ...
        try {
            if (this.isDryRun()) {
                ...
                if (options.driver)
                    this.log(`[DRY RUN] Driver: ${options.driver}`, 'info');
                return;
            }
    ...
    async removeNetwork(name, options) {
        await this.initializeConfig(options);
        const s = spinner();
        ...
        try {
            if (this.isDryRun()) {
                ...
                if (options.advertiseAddr)
                    this.log(`[DRY RUN] Advertise: ${options.advertiseAddr}`, 'info');
                return;
            }
    ...
    async pruneDocker(options) {
        ...
        try {
            if (this.isDryRun()) {
                ...
                if (options.volumes)
                    this.log('[DRY RUN] Would also remove volumes', 'info');
                return;
            }
...
export default function dockerCommand(program) {
