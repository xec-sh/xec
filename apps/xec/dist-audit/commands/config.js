...
export class ConfigCommand extends BaseCommand {
    ...
    async listTargets() {
        ...
        if (targets.hosts && Object.keys(targets.hosts).length > 0) {
            ...
            for (const [name, host] of Object.entries(targets.hosts)) {
                content += prism.cyan(`  🖥️  ${name}`) + prism.dim(` (${host.host}:${host.port || 22})`) + '\n';
            }
        }
        if (targets.containers && Object.keys(targets.containers).length > 0) {
            ...
            for (const [name, container] of Object.entries(targets.containers)) {
                content += prism.blue(`  🐳 ${name}`) + prism.dim(` (${container.container || container.image})`) + '\n';
            }
        }
        if (targets.pods && Object.keys(targets.pods).length > 0) {
            ...
            for (const [name, pod] of Object.entries(targets.pods)) {
                content += prism.yellow(`  ☸️  ${name}`) + prism.dim(` (${pod.namespace || 'default'}/${pod.pod})`) + '\n';
            }
        }
        if (!content.trim()) {
    ...
    async promptSSHConfig(currentConfig) {
    ...
    async promptDockerConfig(currentConfig) {
    ...
    async promptK8sConfig(currentConfig) {
    ...
    async runDoctor(options) {
        ...
        if (!config.description) {
            ...
            if (isCancel(config.description))
        ...
        if (!config.secrets && defaultConfig.secrets) {
            config.secrets = {
                provider: defaultConfig.secrets.provider,
    ...
    async saveConfig(config) {
        this.configManager.merged = config;
    ...
    async deleteTargetWithName(name) {
        const config = await this.configManager.load();
        const allTargets = [
            ...Object.keys(config.targets?.hosts || {}),
            ...Object.keys(config.targets?.containers || {}),
    ...
    async setVarWithKeyValue(key, value) {
        await this.setVar();
    }
    async deleteVarWithKey(key) {
        await this.deleteVar();
    }
    async importVarsFromFile(file) {
        await this.importVars();
    }
    async exportVarsToFile(file) {
        await this.exportVars();
    }
    async viewTaskWithName(name) {
        await this.viewTask();
    }
    async deleteTaskWithName(name) {
        await this.deleteTask();
    }
}
