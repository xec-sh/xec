...
class ProjectInspector {
    ...
    displayTable(results, type) {
        ...
        const output = data.map((row) => row.map((cell) => String(cell ?? '').padEnd(20)).join(' ')).join('\n');
        console.log(output);
        if (this.options.verbose) {
    ...
    displaySystemDetails(result) {
        ...
        switch (result.name) {
            ...
            case 'os':
                ...
                console.log(`${prism.cyan('Load Average:')} ${data.loadavg.map((n) => n.toFixed(2)).join(', ')}`);
                if (data.productName) {
                    console.log(prism.bold('\nmacOS:'));
            ...
            case 'environment':
                ...
                if (data.xecEnv.XEC_HOME)
                    console.log(`  ${prism.cyan('XEC_HOME:')} ${data.xecEnv.XEC_HOME}`);
                if (data.xecEnv.XEC_DEBUG)
                    console.log(`  ${prism.cyan('XEC_DEBUG:')} ${data.xecEnv.XEC_DEBUG}`);
                if (data.xecEnv.XEC_PROFILE)
                    console.log(`  ${prism.cyan('XEC_PROFILE:')} ${data.xecEnv.XEC_PROFILE}`);
                console.log(prism.bold('\nPATH (first 10):'));
        ...
        switch (result.type) {
            case 'task':
                return [
                    ...
                    result.data?.params?.length > 0 ? result.data.params.map((p) => p.name).join(', ') : prism.dim('None'),
                ];
            case 'target':
                return [
                    prism.cyan(result.name),
    ...
    formatTargetDetails(target) {
        ...
        if (config.container)
            parts.push(`container: ${config.container}`);
        if (config.pod)
            parts.push(`pod: ${config.pod}`);
        if (config.namespace)
            parts.push(`ns: ${config.namespace}`);
        return parts.join(', ') || prism.dim('No details');
    }
    formatValue(value, maxLength) {
        if (value === null)
            return prism.dim('null');
        if (value === undefined)
            return prism.dim('undefined');
        let str;
        if (typeof value === 'string') {
            str = value;
        }
...
export function inspectProject(type, name, options) {
    const inspector = new ProjectInspector(options);
    return inspector.initialize().then(() => {
        if (!type && !name) {
            ...
            return inspector.inspect(type, name);
        }
    });
}
