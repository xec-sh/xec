...
export class SecretsCommand extends ConfigAwareCommand {
    ...
    async getSecretManager() {
        ...
        const declared = this.configManager.getConfig().secrets?.provider;
        if (declared && !SecretManager.isSupported(declared)) {
            throw new Error(`Secrets provider '${declared}' is not supported. Supported providers: local, env, git`);
        }
