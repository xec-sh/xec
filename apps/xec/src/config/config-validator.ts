...
export class ConfigValidator {
  ...
  private validateVersion(version: string): void {
    ...
    if (!/^\d+\.\d+$/.test(version)) {
      this.addError('version', `Invalid version format: ${version}. Expected: major.minor (e.g., 2.0)`);
    }
    ...
    if (!major || major < 2) {
      this.addError('version', `Version ${version} is not supported. Minimum version: 2.0`);
    }
  }
