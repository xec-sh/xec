...
export async function loadCommands(program: Command): Promise<void> {
  ...
  // Load dynamic commands from .xec/commands if available
  const dynamicCommandsDir = join(process.cwd(), '.xec/commands');
  if (fs.existsSync(dynamicCommandsDir)) {
    const files = fs.readdirSync(dynamicCommandsDir);
    for (const file of files) {
      if (file.endsWith('.js') || file.endsWith('.mjs') || file.endsWith('.ts')) {
        try {
          const commandPath = join(dynamicCommandsDir, file);
          const module = await import(commandPath);
          ...
          if (process.env['XEC_DEBUG']) {
            console.error(`Failed to load dynamic command ${file}:`, error);
          }
        }
      }
    }
...
async function runScriptDirectly(scriptPath: string, args: string[], options: any) {
  const scriptModule = await import('./commands/script.js');
